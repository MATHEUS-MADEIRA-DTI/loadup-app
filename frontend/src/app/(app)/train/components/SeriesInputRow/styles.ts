...
export const StyledSeriesRow = styled.div<{ $logged: boolean }>`
  background: ${({ $logged, theme }) => $logged ? theme.colors.successContainer : theme.colors.background};
...
export const StyledSeriesTypeBadge = styled.span<{ $bg: string; $text: string }>`
  font-size: 11px;
  font-weight: 600;
  padding: 3px 8px;
...
export const StyledCheckBtn = styled.button<{ $logged: boolean }>`
  ...
  border-radius: 50%;
  border: none;
  background: ${({ $logged, theme }) => $logged ? theme.colors.success : theme.colors.outlineVariant};
  color: ${({ $logged, theme }) => $logged ? theme.colors.onPrimary : theme.colors.onSurfaceMuted};
  display: flex;
  align-items: center;
  justify-content: center;
